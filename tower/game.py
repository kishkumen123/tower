...
def app_response(res, status):
    return Response(
        response=json.dumps(res), status=status, content_type="application/json"
    )
...
def game_new():
    ...
    try:
        cursor.execute(
            "insert into game(data) values (?)", (json.dumps(default_board_state),)
        )
    except sqlite3.OperationalError as e:
        res = {"error": e.args[0]}
...
def game_reset():
    ...
    cursor.execute(
        "update game set data = ? where id == ?", (json.dumps(selection_data), str(_id))
    )
    db.commit()
    cursor.close()
...
def game_move():
    ...
    if not valid_data(_id, src, dst):
        res = {
            "error": "Expecting json body {'id': int, 'src': int, 'dst': int} where src and dst are in range of (1 - 3)"
        }
        return app_response(res, 400)
...
def game_solved():
    ...
    if result != selection_data["solved"]:
        selection_data["solved"] = result
        cursor.execute(
            "update game set data = ? where id == ?",
            (json.dumps(selection_data), str(_id)),
        )
    db.commit()
    cursor.close()
