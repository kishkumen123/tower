...
def show_board_state(_id):
    result = requests.get("http://127.0.0.1:5000/board_state", params={"id": _id})
...
def delete_one(_id):
    result = requests.delete("http://127.0.0.1:5000/delete", params={"id": _id})
    return result

def delete_all():
    result = requests.delete("http://127.0.0.1:5000/delete_all")
    return result

game = new_game()
game_id = game["id"]
board = game["board"]
show_board_state(game_id)
